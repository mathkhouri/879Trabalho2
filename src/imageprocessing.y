...
void parse(int argc, char* argv[], struct user_parameters* params)
{
    ...
    for(i=1; i<argc; i++) {
        if(!strcmp(argv[i], "-t")){
            ...
            printf("-a : Choose algorithm (leaving blank will run type sequential)\n(Options for type) 0 - sequential, 1 - with threads, 2- processo \n");
            printf("-m : modo de varredura ( 0 - por linhas, 1 - colunas\n");
            exit(EXIT_SUCCESS);
            ...
            if (++i < argc)
                params->modo_varredura = atoi(argv[i]);
            else {
            ...
            if (++i < argc)
                if(atoi(argv[i]) == 2)
                    params->type = PROCESSO;
                else if(atoi(argv[i])== 0) params->type = SEQUENCIAL;
                else params->type = THREADS;
            else {
